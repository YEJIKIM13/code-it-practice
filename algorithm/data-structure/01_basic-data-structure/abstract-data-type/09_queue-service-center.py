...
class CustomerServiceCenter:
    ...
    def process_complaint(self):
        ...
        # 가장 먼저 대기 큐가 비어있는지 확인
        if self.queue:  # 대기 중인 문의가 있는지 확인
            # 가장 오래된 문의 먼저 처리
            complaint = self.queue.popleft()  # 가장 앞의 데이터를 삭제하는 동시에 큐에서 삭제하는 데이터 리턴
    ...
    def add_complaint(self, name, email, content):
        ...
        """구체적인 문의 내용을 받아서 문의를 생성하고 접수 대기 큐에 추가"""
        # 파라미터로 넘겨 받은 데이터를 이용해 문의 인스턴스를 생성
        new_complaint = CustomerComplaint(name, email, content)  # 새 문의 인스턴스 생성
        # 새롭게 만든 이 문의 사항을 대기 큐에 추가
